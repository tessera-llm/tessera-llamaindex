...
class TestOpenAI:
    def test_default(self):
        cfg = tessera_openai_config(api_key='tsr_test')
        assert cfg['api_base'] == f'{TESSERA_BASE_URL}/v1/openai'
        assert cfg['default_headers'] == {'x-tessera-api-key': 'tsr_test'}
...
class TestAnthropic:
    def test_default(self):
        cfg = tessera_anthropic_config(api_key='tsr_test')
...
class TestMistral:
    def test_default(self):
        cfg = tessera_mistral_config(api_key='tsr_test')
        ...
        assert cfg['additional_kwargs'] == {'http_headers': {'x-tessera-api-key': 'tsr_test'}}


class TestGroq:
    def test_default(self):
        cfg = tessera_groq_config(api_key='tsr_test')
...
class TestGenericDispatcher:
    ...
    def test_provider_routing(self, provider, base_url_key):
        cfg = tessera_config(provider=provider, api_key='tsr_test')
        assert cfg[base_url_key] == f'{TESSERA_BASE_URL}/v1/{provider}'

    def test_unknown_provider_raises(self):
        with pytest.raises(ValueError, match='Unknown provider'):
            tessera_config(provider='not-a-provider', api_key='tsr_test')  # type: ignore[arg-type]
